...
class Tx_Orthodox_Controller_DayController extends Tx_Extbase_MVC_Controller_ActionController {
	private $perehod;
	private $neperehod;
	private $sunday_matins_gospels;
	private $zachala;
	private $saints;
	private $lect;
	private $prazdnik_title;
	private $skip_rjad;
	private $no_liturgy;
	private $day_of_week_number;
	
	private $day_of_week_names = array('воскресение','понедельник','вторник','среду','четверг','пятницу','субботу');

	private function getDayAfter($date,$day_number = 1, $sh_times = 0, $no_jump_if_same_day = 0){
	...
	private function getDayBefore($date,$day_number = 1,$sh_times = 0){
		$day_stamp = strtotime($date);
		$current_day_number = date('N', $day_stamp);
		if($current_day_number == 7)
			$current_day_number = 0;
	...
	private function getDayNearest($date,$day_number = 1){
		$day_stamp = strtotime($date);
	...
	private function getKey($key,$d_stamp){
		...
		if(preg_match("/(\d\d\/\d\d)(.)?(\w)?#?(\d)?/u",$key,$out)){
			...
			$sh_sign = $out['2'];//operation sign
			$sh_dayn = $out['3'];//day number,0 - sunday
			$sh_times = $out['4'];
	...
	private function getNeperehod($date_stamp){
	...
	private function process_perehods($week,$day_of_week_number,$gospel_shift,$week_old,$date_stamp_o,$year,$easter_stamp){
	...
	private function formatReading($arr,$weekend){
		foreach($arr as $array){
			...
			//order services
			$nr_or['Утреня'] = $nr['Утреня'];
			$nr_or['1-й час'] = $nr['1-й час'];
			$nr_or['3-й час'] = $nr['3-й час'];
			$nr_or['6-й час'] = $nr['6-й час'];
			$nr_or['9-й час'] = $nr['9-й час'];
			$nr_or['Литургия'] = $nr['Литургия'];
			$nr_or['Вечерня'] = $nr['Вечерня'];
			$nr_or['На освящении воды'] = $nr['На освящении воды'];
			...
			foreach($nr2 as $rtitle => $readings){
				...
				foreach(explode(";",$readings) as $reading){
					...
					if($r = $this->zachala[$reading]){
						$flag = true;
						$fl = true;
						$str .= '&nbsp;<a class="reading" href="http://bible.psmb.ru/bible/book/'.$reading.'/">'.$r.'</a>&nbsp;&nbsp;';
					}
				}
				if($flag)
	...
	private function check_skip_rjad($saints){
	...
	$GLOBALS['TSFE']->additionalHeaderData['orthodox'] = '
	<script src="/typo3conf/ext/orthodox/Resources/Public/JavaScript/orthodox.js" type="text/javascript"></script>';
...
var d = new Date(".substr($date,0,4).", ".(substr($date,4,2)-1).", ".substr($date,6,2).");
...
onSelect: function(dateText, inst) {
	...
	var url = 'http://c.psmb.ru/pravoslavnyi-kalendar/date/'+dateText;
	document.title = date+' - Православный календарь';
	";
	if($_GET["debug"]){
		$block .= "if(history.pushState){ history.pushState(null, date+' - Православный календарь', url+'?debug=1'); }
		...
		$block .= "if(history.pushState){ history.pushState(null, date+' - Православный календарь', url); }
	...
	$block .= "
		...
		$css_block = '<style>
		.ui-datepicker td.highlight-12 a{
			background: red;
		}		
		.ui-datepicker td.highlight-great a{
			background: #FF5050;
			color: white;
		}
		...
		.ui-datepicker td.highlight-post a{
			background: #E3D1FF;
		}
		...
		$filename = $_SERVER['DOCUMENT_ROOT'].'/typo3conf/ext/orthodox/Data/cache_perehod.csv';
		$gid = 4;
		if($_GET["debug"])
			unlink($filename);
		if(!file_exists($filename)){
		...
		$filename = $_SERVER['DOCUMENT_ROOT'].'/typo3conf/ext/orthodox/Data/cache_neperehod.csv';
		$gid = 0;
		if($_GET["debug"])
			unlink($filename);
		if(!file_exists($filename)){
		...
		$file = fopen('typo3conf/ext/orthodox/Data/static_sunday_matins_gospels.csv', 'r');
		while (($line = fgetcsv($file)) !== FALSE) {
			$this->sunday_matins_gospels[$line[0]] = $line[1];
		}
		fclose($file);	
		
		$filename = $_SERVER['DOCUMENT_ROOT'].'/typo3conf/ext/orthodox/Data/cache_saints.csv';
		$gid = 5;
		if(!file_exists($filename)){
			file_put_contents($filename, file_get_contents($google_url.$gid));
		}
		...
		$filename = $_SERVER['DOCUMENT_ROOT'].'/typo3conf/ext/orthodox/Data/cache_lect.csv';
		$gid = 19;
		if(!file_exists($filename)){
			file_put_contents($filename, file_get_contents($google_url.$gid));
		}
		$file = fopen($filename, 'r');
		while (($line = fgetcsv($file)) !== FALSE) {
			$this->lect[$line[0]][0] = $line[1];
			$this->lect[$line[0]][1] = $line[2];
		}
		fclose($file);
	}
	/**
	...
	public function listAction($date = null) {
		...
		if($_GET["brother"]){
			...
			if($d[0] == 0)
				$d = substr($d,1);
			echo "<html><head><title>Лекционарий</title><style>body{font-size:300%;}</style></head><body>";
			echo "Утреня: ".$this->lect[$d][0]."<br>";
			echo "Вечерня: ".$this->lect[$d][1]."<br>";
			die("</body></html>");
		}
		...
		$GLOBALS['TSFE']->additionalHeaderData['99'] = "<title>".date('d.m.Y', $date_stamp)." - Православный календарь</title>";
		...
		$monday_after_krest = $this->getDayAfter('27-09-'.$year);
		$week_after_krest = datediff('ww', $easter_stamp, $monday_after_krest, true) + 1;
		$krest_diff = 25 - $week_after_krest;	
		$monday_18th_week = strtotime("+24 weeks 1 day", $easter_stamp);

		if($date_stamp >= $monday_after_krest){//in any case, start to read Luke only on Monday after Krestovozdvijenije
		...
		if($date_stamp >= $monday_after_prosv){ // reset gospel shift if after $monday_after_prosv
			$gospel_shift = 0;
			$debug .= "gospel shift reset!";
		}
		if($date_stamp >= $monday_after_prosv){ // TODO: research, this should start from $week_after_prosv
			if((($week_to_easter == -12)&&($this->day_of_week_number != 0))||(($week_to_easter == -11)&&($this->day_of_week_number == 0))){
				$week_to_easter = $week_to_easter - 14;
			}else if($week_to_easter <= -12){
				$week_to_easter = $week_to_easter + 1;
			}
		...
		$debug .= "Понедельник по крестовоздвижению: ".date("Ymd",$monday_after_krest) . "| 17-я неделя " . date('Ymd',$monday_18th_week) . "<br/>";
		...
		if($this->day_of_week_number == 0){
			require($_SERVER['DOCUMENT_ROOT'].'/typo3conf/ext/orthodox/Data/static_sunday_troparion.php');
		...
		//$reading_real = $row['reading'];
		//preg_match_all("/,\s(\d{1,3})\sзач\./u",$reading_real,$match);
		//$real_zachala = $match[1];
			
		if($_GET["debug"]){	
			//check readings
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*','tx_biblequote_days','date='.date('Ymd',$date_stamp));
			$row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res);
			$reading_real = $row['reading'];
			preg_match_all("/,\s(\d{1,3})\sзач\./u",$reading_real,$match);
			$real_zachala = $match[1];
			
			foreach($real_zachala as $rz){
				if(strpos($reading_str, $rz." зач.") === false){
					$debug_r .= "<br/><span style='color:red;'>$rz FAIL</span>";
				} else {
					$debug_r .= "<br/><span style='color:green;'>$rz match!</span>";
				}
			}
			$debug .= $debug_r;
		}
		if($_GET["debug"])
			$reading_str .= "<br/>Проверочное патриархийное чтение: ".$reading_real;
		...
		//$assignArray['prazdnik_title'] = $this->prazdnik_title;
		$this->view->assignMultiple($assignArray);
		$this->view->assign('json',json_encode($assignArray));
