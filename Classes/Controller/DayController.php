...
class Tx_Orthodox_Controller_DayController extends Tx_Extbase_MVC_Controller_ActionController
{
	...
	private function getStaticData($datestamp) {
		$d = date("Y-m-d", $datestamp);
		$filename = $_SERVER['DOCUMENT_ROOT'] . '/typo3conf/ext/orthodox/Data/processed/' . $d;
	...
	{
		$js_block                                     = '
		<script type="text/javascript">var d = new Date(' . substr($date, 0, 4) . ', ' . (substr($date, 4, 2) - 1) . ', ' . substr($date, 6, 2) . ');</script>
		<script src="/typo3conf/ext/orthodox/Resources/Public/JavaScript/orthodox.js" type="text/javascript"></script>
		<script src="/typo3conf/ext/orthodox/Resources/Public/JavaScript/main.js" type="text/javascript"></script>
		';
		$GLOBALS['TSFE']->additionalHeaderData['100'] = $js_block;
		$css_block                                    = '<style>
		...
		.ui-datepicker td.highlight-post.lent7 a {
			background: #FFFBD1;
		}
	...
	public function listAction($date = null)
	{
		...
		if ($staticData) {
			...
			$jsonArray = array(
				...
				"readings" => $staticData['readings'],
				"saints" => $staticData['saints'],
				"prayers" => $assignArray['prayers'],
				"seromns" => $assignArray['sermons'],
				"lent" => $assignArray['lent'],
				"comment" => $assignArray['comment']
			);
		}

		$this->view->assignMultiple($assignArray);
		$this->view->assign('json', json_encode($jsonArray));	
	}
}
?>
